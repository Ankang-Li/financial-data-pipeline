...
These are the questions this project is built to answer, stated plainly.

**1. Why build a pipeline instead of using a Bloomberg terminal / a vendor feed?**
Because I wanted to *own the provenance and the validation*, not rent them. A terminal gives
you a number; it does not tell you which unit convention the number used, whether the
source double-reported a day, or whether two vendors disagree by 100×. This project makes
those questions answerable for the specific series I research, and it is reproducible by
anyone who clones the repo — no terminal license required.

**2. What does "validation" mean to you?**
Not dropping bad rows. It means recording, per dataset and per run, a list of explicit
checks (schema, time-series, financial logic, cross-source) with a severity, and routing
`ERROR` to quarantine and `WARNING` to a stored flag. The goal is an auditable trail, not a
silent `dropna()`.

**3. Why keep raw and normalized data separate?**
The raw layer is a legal record: bytes exactly as received, checksummed, never rewritten.
If a normalization rule is wrong, I can fix it and re-derive everything from raw. If raw and
normalized lived in one place, a bug in normalization would be irreversible.

**4. How do you make a result reproducible?**
Run-context + config fingerprint + checksummed raw snapshots + versioned canonical schemas +
a metadata/validation log in the warehouse. Any number traces back to a run id, a source
snapshot, and the exact validation that judged it.

**5. Why DuckDB?**
Single file, no server, real SQL, and it scales from a laptop to large extracts without a
configuration change. For research it removes the " stand up a database first" tax while
keeping the data queryable the same way a real warehouse would be.

**6. How do you handle a source changing its schema?**
Each source has a registered expected schema; drift (missing required → ERROR, unexpected →
WARNING) is caught at ingestion. Canonical tables are versioned, and the version travels
with every load, so old extracts stay interpretable and a contract change is a visible event
rather than a silent break.

**7. How does this feed quant research or a RAG system?**
Downstream code reads canonical datasets through one API and never sees a provider's quirks,
so a strategy or a model trains on consistent identifiers and units. For RAG, the
`validation_results` and `dataset_metadata` tables are structured, citable context: a
retriever can answer "which US 10y source disagreed, and when?" from the warehouse instead
of from prose.
...
> Built a source-agnostic financial data pipeline (Python, Pandas, DuckDB, Pydantic) covering
> ingestion, validation, normalization and a query layer; demonstrated cross-source
> consistency checks that catch unit errors a naive feed would pass through.
>
> Designed an immutable, checksummed raw layer with run-level provenance so any research
> number is reproducible from a run id and source snapshot; shipped schema-drift detection
> and a severity-based validation engine that quarantines unusable data.
>
> Implemented an offline-first design (committed synthetic samples + replay adapter) so the
> full pipeline and test suite run with no network or API key, validated by CI on Python
> 3.11/3.12.
